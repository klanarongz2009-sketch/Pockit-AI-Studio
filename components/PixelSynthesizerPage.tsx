...
export const PixelSynthesizerPage: React.FC<{
    onClose: () => void;
        ...
        <PageWrapper>
            ...
            <main id="main-content" className="w-full max-w-2xl flex-grow flex flex-col items-center justify-center gap-6 p-4">
                ...
                <div className="relative w-full max-w-2xl h-48 bg-black border-4 border-brand-light shadow-pixel select-none" aria-label="คีย์บอร์ดซินธิไซเซอร์">
                    {keys.filter(k => k.type === 'white').map((key, index) => (
                        <button
                            key={key.note}
                     ...
                     {keys.filter(k => k.type === 'black').map((key) => {
                        ...
                        return (
                             <button
                                ...
                                className={`absolute top-0 h-2/3 w-[6%] bg-black border-2 border-brand-light transition-colors z-10 ${activeNote === key.note ? 'bg-gray-600' : 'hover:bg-gray-800'}`}
                                style={{ left: `${whiteKeysBefore * 8.333 - 3}%` }}
                                aria-label={`เล่นโน้ต ${key.note}`}
                            />
                        )
                     })}
