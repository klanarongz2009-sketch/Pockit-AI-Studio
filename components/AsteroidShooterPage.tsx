...
export const AsteroidShooterPage: React.FC<AsteroidShooterPageProps> = ({ onClose, addCredits }) => {
    ...
    useEffect(() => {
        ...
        function endGame() {
            gameOver = true;
            addCredits(Math.floor(score / 100));
            overlayTitle.textContent = "GAME OVER"; overlayMessage.innerHTML = `Final Score: <span class="text-xl text-yellow-400">${score}</span><br>Better luck next time, pilot!`; startButton.textContent = "RETRY"; overlay.classList.remove('hidden');
        }
