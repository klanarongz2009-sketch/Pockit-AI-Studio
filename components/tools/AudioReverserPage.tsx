...
interface AudioReverserPageProps {
    onClose: () => void;
    playSound: (player: () => void) => void;
}

export const AudioReverserPage: React.FC<AudioReverserPageProps> = ({ onClose, playSound }) => {
    ...
    const handleDownload = useCallback(async () => {
        ...
        try {
            ...
            a.href = url;
            a.download = `reversed_${uploadedFile?.name.split('.').slice(0, -1).join('.')}.wav`;
    ...
    return (
        <PageWrapper>
            ...
            <PageHeader title="Audio Reverser" onBack={onClose} />
            <main id="main-content" className="w-full max-w-lg flex flex-col items-center gap-6 font-sans">
                <p className="text-sm text-center text-text-secondary">Upload an audio or video file to extract and reverse its audio track. A fun way to find hidden messages!</p>
                {!uploadedFile ? (
                    ...
                    <div className="w-full space-y-4">
                        <div className="bg-black/40 p-4 border-2 border-brand-light/50">
                            {uploadedFile.type.startsWith('video/') ? (
                                <video src={previewUrl!} controls className="w-full max-h-60" />
                            ) : (
                        ...
                        <button onClick={handleReverse} disabled={isLoading} className="w-full flex items-center justify-center gap-3 p-4 bg-brand-magenta text-white border-4 border-border-primary shadow-pixel disabled:bg-gray-500">
                            <ReverseIcon className="w-6 h-6" /> {isLoading ? 'Reversing...' : 'Reverse Audio'}
                        </button>
                    </div>
                )}
                <div className="w-full min-h-[8rem] p-4 bg-surface-1 border-4 border-border-primary flex flex-col items-center justify-center">
                    {isLoading && <LoadingSpinner text="Reversing audio..." />}
