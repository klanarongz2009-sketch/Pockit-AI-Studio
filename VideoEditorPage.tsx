...
export const VideoEditorPage: React.FC<VideoEditorPageProps> = ({ onClose, playSound, isOnline }) => {
    ...
    const generateSubtitles = useCallback(async (mediaFile: File) => {
         try {
            ...
            const vttContent = await generateSubtitlesFromVideo(base64Data, mediaFile.type, 'th-TH');
                         ...
                         {activeTool === 'subs' && (
                            <div className="p-4 bg-black/20 border-2 border-brand-light/50">
                                <label className="flex items-center gap-3 cursor-pointer">
                                    ...
                                    />
                                    <span className="text-sm font-press-start">สร้างคำบรรยายอัตโนมัติ (ภาษาไทย)</span>
                                </label>
                                <p className="text-xs text-brand-light/70 mt-2 pl-9">AI จะวิเคราะห์เสียงในไฟล์และสร้างไฟล์คำบรรยาย (VTT) ให้โดยอัตโนมัติ</p>
                            </div>
                        )}
                    </div>
                
                    {/* Toolbar */}
                    <div className="w-full grid grid-cols-4 gap-2 p-2 bg-black/30 border-2 border-brand-light/50">
                        <button 
